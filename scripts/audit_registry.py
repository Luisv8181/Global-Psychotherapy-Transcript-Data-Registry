...
from src.registry.records import RecordLoadError, load_records

p = argparse.ArgumentParser(description=__doc__.splitlines()[0])
p.add_argument("--json", action="store_true", help="print JSON instead of Markdown tables")
args = p.parse_args()
try:
    records = load_records(Path(__file__).resolve().parents[1])
except RecordLoadError as exc:
    raise SystemExit(f"error: {exc}\nRun scripts/validate_registry.py for a full report.")
